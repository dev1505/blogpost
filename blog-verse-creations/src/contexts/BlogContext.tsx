...
interface BlogContextType {
  ...
  getBlogsByUserName: (username: string) => Promise<BlogPost[]>;
...
export const BlogProvider = ({ children }: { children: ReactNode }) => {
  ...
  const getBlogsByUserName = async (username: string) => {
    const response = await CommonApiCall({
      url: `${fastapi_backend_url}/get/user/blogs/${username}`,
      type: 'get',
      publicPage: true
    });
  ...
  return (
    <BlogContext.Provider
      value={{ posts, setPosts, addBlog, editBlog, getBlogsByUserName, getBlogById, getBlogsByUser, generateBlog }}
    >
      {children}
    </BlogContext.Provider>
  );
